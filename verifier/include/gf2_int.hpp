#ifndef MASKSCRAPE_GF2_INT_H
#define MASKSCRAPE_GF2_INT_H

#include "llvm/Support/raw_ostream.h"
#include <limits>
#include <llvm/Support/FormatProviders.h>

namespace maskscrape {

template <typename T, T Mod> struct gf2_int {
  using underlying_type = T;

  underlying_type v;

  gf2_int() : v(0) {}
  gf2_int(T v) : v(v) {}

  bool operator==(const gf2_int &) const = default;
  auto operator<=>(const gf2_int &) const = default;

  friend gf2_int operator+(gf2_int x, gf2_int y) { return x.v ^ y.v; }

  friend gf2_int &operator+=(gf2_int &x, gf2_int y) {
    x.v ^= y.v;
    return x;
  }

  friend gf2_int operator*(gf2_int x, gf2_int y) {
    T p = 0, a = x.v, b = y.v;

    while (a && b) {
      if (b & 1)
        p ^= a;
      b >>= 1;

      if (a & 0x80)
        a = (a << 1) ^ Mod;
      else
        a <<= 1;
    }

    return p;
  }

  friend gf2_int &operator*=(gf2_int &x, gf2_int y) { return x = x * y; }

  explicit operator T() const { return v; }

  static gf2_int rand() {
    return std::rand() % (std::numeric_limits<T>::max() + 1);
  }
};

struct gf2_int8 : gf2_int<unsigned char, 0x1b> {
  using base_type = gf2_int<unsigned char, 0x1b>;

  using base_type::base_type;

  gf2_int8(const base_type &v) : base_type(v) {}

  // log table using 0xe5 as the generator
  static inline unsigned char ltable[256] = {
      };

  // anti-log table
  static inline unsigned char atable[256] = {
      };

  static unsigned char multiply(unsigned char a, unsigned char b) {
    int s = ltable[a] + ltable[b];

    s %= 255;
    s = atable[s];

    int q = s;
    s = (a == 0 ? 0 : q);
    (b == 0 ? s : q) = 0;

    return s;
  }

  static unsigned char inverse(unsigned char in) {
    if (in == 0)
      return 0;
    else
      return atable[(255 - ltable[in])];
  }

  static unsigned char divide(unsigned char a, unsigned char b) {
    int s = ltable[a] + 255 - ltable[b];

    s %= 255;
    s = atable[s];

    int q = s;
    s = (a == 0 ? 0 : q);
    (b == 0 ? s : q) = 0;

    return s;
  }

  friend gf2_int8 operator/(gf2_int8 a, gf2_int8 b) {
    return divide(static_cast<underlying_type>(a),
                  static_cast<underlying_type>(b));
  }
};

template <typename T, T Mod>
llvm::raw_ostream &operator<<(llvm::raw_ostream &Stream,
                              const gf2_int<T, Mod> &V) {
  return Stream << static_cast<int>(V.v);
}

} // namespace maskscrape

#endif // MASKSCRAPE_GF2_INT_H
